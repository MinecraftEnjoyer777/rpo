...
#define LOG_INFO(...) __android_log_print(ANDROID_LOG_INFO, "rpolab_ndk", __VA_ARGS__)
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_example_rpolab_MainActivity_encrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data) {
...
jbyte * pdata =env->GetByteArrayElements(data, 0);
               ...
               extern "C" JNIEXPORT jbyteArray JNICALL
               Java_com_example_rpolab_MainActivity_decrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data) {
               jsize ksz = env->GetArrayLength(key);
               jsize dsz = env->GetArrayLength(data);
